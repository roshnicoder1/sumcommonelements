...
public class SumOfCommonElementsAppTest {
  ...
  public void givenUserEntersValidArrayValuesWhenCommonELemnetsFoundThenDisplaysRequiresSum() {
      ...
      String MSG1 = "Application Should display the sum of common elements of the given two arrays";
      String actual = myOutStream.toString().trim().replaceAll("\\s+", " ");
      Assertions.assertEquals(output.replaceAll("\\s+", " "), actual, MSG1);
  }
    ...
    public void givenUserEntersInValidArraySizeWhenArrayConstructedThenDisplaysNoElements() {
        ...
        System.setIn(myInputStream);
        SumOfCommonElements.main(null);
        String MSG1 = "Application Should display No elements in arrays for empty arrays";
        String actual = myOutStream.toString().toLowerCase().trim().replaceAll("\\s+", " ");
        Assertions.assertEquals(output.toLowerCase().replaceAll("\\s+", " "), actual, MSG1);
    }
}
